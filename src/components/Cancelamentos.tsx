...
const Cancelamentos = ({ embedded = false, initialDe, initialAte, initialGroupBy, initialPlanoDe }: CancelamentosProps = {}) => {
  ...
  const grouped = useMemo(() => {
    ...
    const keyOf = (r: Row) => {
      if (groupBy === "motivo") return (r.Ds_Motivo_Cancelamento ?? r.MOTIVO_CANCELAMENTO ?? "").toString().trim() || "SEM MOTIVO";
      ...
      {rows && (
        <>
          <div className="flex items-center justify-between text-xs text-muted-foreground mb-3">
            ...
            <span>
              <span className="font-semibold text-foreground tabular-nums">{fmtInt(total)}</span> cancelamento(s) ·{" "}
