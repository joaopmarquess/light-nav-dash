...
export function drawReportHeading(
  ...
  doc.setTextColor(...PDF_COLORS.navy);
  doc.text(title, cx, 23, { align: "center", baseline: "middle" });

  doc.setFillColor(...PDF_COLORS.groupFill);
  doc.rect(marginL, 25.6, pageW - marginL - marginR, 6, "F");
  doc.setFontSize(8.5);
  doc.setTextColor(...PDF_COLORS.navy);
  doc.text(plano, cx, 28.7, { align: "center", baseline: "middle" });

  doc.setFontSize(9.5);
  doc.setTextColor(...PDF_COLORS.text);
  doc.text(secao, cx, 35.5, { align: "center", baseline: "middle" });
